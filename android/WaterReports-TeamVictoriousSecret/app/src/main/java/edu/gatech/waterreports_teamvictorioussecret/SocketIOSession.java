...
public class SocketIOSession implements Session {
    private static SocketIOSession instance;
    ...
    public boolean removeListener(int listenerId) {
        ...
        if (updateListeners.containsKey(listenerId)) {
            detachListeners.remove(listenerId);
            return true;
        }
    ...
    @Override
    public void register(String email, String password, WorkerType type) {
        JSONObject request = new JSONObject();
        try {
            request.put("username", email);
            request.put("password", password);
            request.put("name", type.toString());
    ...
    public static SocketIOSession getInstance() {
        if (instance == null) {
            //create and bind instance
            instance = new SocketIOSession();
        }
        return instance;
    }
        ...
        this.socket.on(Socket.EVENT_CONNECT, new Emitter.Listener() {
            ...
            public void call(Object... args) {
                ...
                try {
                    ...
                    for (Session.AttachListener listener : SocketIOSession.this.attachListeners.values()) {
